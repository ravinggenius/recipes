...
var ANSI_CODES, APP_COFFEE, APP_HTML, APP_JS, APP_STYL, BASE_STYL, GITIGNORE_COFFEE, GITIGNORE_JS, MAKEFILE_COFFEE, README, RESET_STYL, SERVER, SERVER_COFFEE, SERVER_JS, abort, basename, createProject, derby, emptyDirectory, exec, fs, join, makeCallback, mkdir, mkdirp, newProject, packageJson, printUsage, program, render, resolve, style, styleTag, writeFile, _404_HTML, _404_STYL, _ref;
...
makeCallback = function(path, callback) {
  return function(err) {
    if (err) {
      throw err;
    }
...
mkdir = function(path, callback) {
  return mkdirp(path, '0755', makeCallback(path, callback));
};

writeFile = function(path, text, callback) {
  return fs.writeFile(path, text, makeCallback(path, callback));
};
...
createProject = function(dir, app, useCoffee) {
  var appScripts, appStyles, appViews, count, dirPath, finish, logComplete, project, scripts, serverScripts, styles, views, wait;
  ...
  return mkdir(dir, function() {
    mkdir(join(dir, 'public', 'img'), wait());
    mkdir(appViews, wait(function() {
      writeFile(join(appViews, 'index.html'), APP_HTML, wait());
      return writeFile(join(views, '404.html'), _404_HTML, wait());
    }));
    mkdir(appStyles, wait(function() {
      ...
      return writeFile(join(styles, 'base.styl'), BASE_STYL, wait());
    }));
    if (useCoffee) {
      mkdir(appScripts, wait(function() {
        return writeFile(join(appScripts, 'index.coffee'), render(APP_COFFEE, {
          app: app
        }), wait());
      }));
      mkdir(serverScripts, wait(function() {
        return writeFile(join(serverScripts, 'index.coffee'), render(SERVER_COFFEE, {
          app: app
        }), wait());
      }));
      ...
      mkdir(appScripts, wait(function() {
        return writeFile(join(appScripts, 'index.js'), render(APP_JS, {
          app: app
        }), wait());
      }));
      mkdir(serverScripts, wait(function() {
        return writeFile(join(serverScripts, 'index.js'), render(SERVER_JS, {
          app: app
        }), wait());
      }));
    ...
    return writeFile(join(dir, 'README.md'), render(README, {
      project: project
    }), wait());
  });
};
