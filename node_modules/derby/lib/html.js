...
var attr, comment, empty, endTag, entityToCode, name, rawTextEnd, rawTextTag, startTag, uncomment, voidElement, _i, _len, _ref;
...
_ref = 'area,base,br,col,command,embed,hr,img,input,keygen,link,meta,param,source,track,wbr'.split(',');
...
exports.isVoid = function(name) {
  return !!voidElement[name];
};

empty = function() {};

exports.parse = function(html, handler) {
  var chars, charsHandler, endHandler, index, last, match, parseChars, parseEndTag, parseStartTag, pos, startHandler, tagName, _results;
  if (handler == null) {
    handler = {};
  }
  ...
  parseStartTag = function(tag, tagName, rest) {
    ...
    return startHandler(tag, tagName.toLowerCase(), attrs, html);
  };
  parseEndTag = function(tag, tagName) {
    return endHandler(tag, tagName.toLowerCase(), html);
  };
  parseChars = function(index, isRawText) {
    var text;
    if (~index) {
    ...
    if (text) {
      return charsHandler(text, isRawText, html);
    }
  };
  html = uncomment(html);
  _results = [];
...
exports.unescapeEntities = function(html) {
  return html.replace(/&([^;]+);/g, function(match, entity) {
    return String.fromCharCode(entity.charAt(0) === '#' ? entity.charAt(1) === 'x' ? parseInt(entity.slice(2, 17)) : parseInt(entity.slice(1)) : entityToCode[entity]);
  });
};
